...
public class ProtocolUtils extends BigIntegerUtil {
   ...
   */
  public static BigInteger crt(BigInteger d1, BigInteger d2, BigInteger x1,
      BigInteger x2) {
    BigInteger y1, y2, n = d1.multiply(d2);
    y1 = (d2.mod(d1)).modInverse(d1);
    y2 = (d1.mod(d2)).modInverse(d2);
    return (d2.multiply(y1).multiply(x1).mod(n)
        .add(d1.multiply(y2).multiply(x2)).mod(n));
  }

  /**
   * See ALGORITHMS 5.2.1. Calculate the square root of <code>val</code> modulo
   * <code>modulus</code>. <code>modulus</code> must be a prime number, and
  ...
  public static BigInteger sqrtMod(BigInteger val, BigInteger modulus) {
    ...
    } while ((b.modPow(modulus.subtract(ONE).shiftRight(1), modulus))
        .compareTo(ONE) == 0);
    ...
    while (K[n] != 0) {
      exp = ONE
          .shiftLeft((lm[0].subtract(BigInteger.valueOf(K[n]))).intValue());
      y = (y.multiply(b.modPow(exp, modulus))).mod(modulus);
      K[++n] = sucheKN(y, lm[1], modulus);
    }
    x = y.modPow(lm[1].add(ONE).shiftRight(1), modulus);
    for (i = n; i > 1; i--) {
      exp = ONE.shiftLeft((lm[0].subtract(BigInteger.valueOf(K[i - 1]))
          .subtract(ONE)).intValue());
  ...
  /*
   * Die folgende Routine ist ein Teil von Algorithmus 5.2.1. Die Zahl p wird
   * als ungerade vorausgesetzt. Der Algorithmus isoliert die ungeraden
   * Faktoren. (u.a.) Die Endbedingung ist: p-1 = m*2^l, l maximal.
   */
  ...
  /*
   * Diese Routine ist ein Teil von Algorithmus 5.2.1. Sie liefert: min { k >= 0
   * | y^((2^k)*m) mod p = 1 }
   */
