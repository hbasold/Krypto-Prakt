...
public class ServerThread extends Thread {
  ...
  /**
   * Konstruktor, uebernimmt eine Socket-Verbindung vom Server, initialisiert
  ...
  public ServerThread(Socket connection, Server server, int cnt, int gameID,
    ...
    try {
      ...
      String log = "player " + (myNumber + 1)
          + ":error creating ServerThread\n";
  ...
  public String receive() throws InterruptedIOException, IOException {
    ...
    if (in.read(c, 0, len) == -1)
      System.out.println("ServerThread " + myNumber
          + " : error reading from socket!");

    /*
     * while (s.equals("")) { while(in.ready()) { result.print(in.readLine());
     * if (in.ready()) result.println(); } result.flush(); w.flush(); s =
     * w.toString(); }
     */
    ...
    while (!isInterrupted()) {
      ...
      try {
        ...
        if (message != null) {
          ...
          System.out.println("sending message from " + myNumber + " to "
              + target);
          // falls target="-1": Nachricht an Server; sonst Nachricht an Spieler
          // x
          if (target >= -1) {
          ...
          String log = "Aborting game! Player " + (myNumber + 1)
              + " has left the game\n";
          System.out.print(log);
          server.log(log, server.getPort());
          interrupt();
