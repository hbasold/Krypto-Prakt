...
/**
 * Die Klasse Client stellt im Grunde das Hauptprogramm dar. Hier werden die
...
public class Client implements IClient {
  ...
  public boolean loadGame(String protocolName) {
    ...
    try {
      ...
      myGUI
          .printOutput("Protokoll-Klasse " + protocolName + " nicht gefunden!");
   ...
   * Danach erzeugt sie ein neues Objekt vom Typ Communicator und uebergibt
   * diesem die Spieldaten sowie den gewuenschten Servernamen/Port.<br>
  ...
  public void connect() {
    ...
    try {
      myCom = new Communicator(host, port, shortName, minPlayer, maxPlayer,
          this);
   ...
   * Server angemeldet ist) und schickt dann, wenn notwendig dem Server, dass er
   * sich beendet. Dann wird der Client beendet.
   */
  public void exit() {
    if (myCom != null)
      myCom.sendTo(-2, String.valueOf(myCom.myNumber()));
  ...
  /**
   * Diese Methode macht das Client Fenster wieder sichtbar, benachrichtigt den
   * Anwender mittels eines popUpFensters ueber den Abbruch des Spiels durch
    ...
    if (args.length > 0) {
      for (int i = 0; i < args.length; i++) {
        ...
        if ((args[i] = args[i].toLowerCase()).startsWith("-port=")) {
          try {
            port = Integer
                .parseInt(args[i].substring(args[i].indexOf("=") + 1));
