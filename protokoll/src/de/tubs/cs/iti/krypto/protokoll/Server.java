...
public class Server {
  ...
  public synchronized void deletePlayer(String data) {
    try {
      ...
      log = "server: deleting player " + player + " in Game " + name + " ID:"
          + gameID + "\n";
      System.out.print(log);
      log(log, port);
      ...
      for (int i = 0; i < games.size(); i++) {
        ...
        if (game.getNameOfTheGame().equals(name) && gameID == game.getGameID()) {
          ...
          if (game.getPlayers().size() == 0) {
            games.remove(i);
            System.out
                .println("server: game " + (i + 1) + " removed from list");
          }
        }
      }
    } catch (NoSuchElementException e) {
  ...
  public synchronized void forward(int gameID, String data) {
    ...
    try {
      ...
      for (int i = 0; i < games.size(); i++) {
        ...
        if (game.getNameOfTheGame().equals(name) && gameID == game.getGameID()) {
          if (target != -1) {
            ServerThread recipient = ((ServerThread) game.getPlayers()
                .elementAt(target));
            recipient.send("1" + msg);
          } else {
            // falls nachricht an server gerichtet ist (player "-1", nachricht
            // "go"),
  ...
  public static void main(String[] args) {
    ...
    if (newport < 1024) { // teste, ob Portnummer gueltig ist
      System.out.println("server: Invalid port number! Using default port "
          + defaultPort);
      newport = defaultPort;
    }
  ...
  /**
   * Diese Methode unterbricht die Verbindung zu den Spielern eines Spiels. Sie
